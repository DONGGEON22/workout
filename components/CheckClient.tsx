...
const MSG_NOT_CHECK_PERIOD =
  "체크 기간이 아닙니다. 이번 주(토요일~금요일)에 해당하는 날에만 기록할 수 있습니다.";
...
export default function CheckClient() {
  ...
  const editable =
    !week || nowMs === 0 ? true : nowMs < new Date(week.weekEnd).getTime();
  ...
  async function toggleDay(dayIndex: number, has: boolean) {
    ...
    try {
      if (has) {
        ...
        if (!res.ok) {
          window.alert(typeof data.error === "string" ? data.error : "취소 실패");
          return;
        }
      } else {
        ...
        const data = await res.json().catch(() => ({}));
        if (!res.ok) {
          window.alert(typeof data.error === "string" ? data.error : "기록 실패");
          return;
        }
        void celebrate();
      }
  ...
  async function submitDayWithPhoto(dayIndex: number, file: File) {
    ...
    try {
      ...
      const data = await res.json().catch(() => ({}));
      if (!res.ok) {
        window.alert(typeof data.error === "string" ? data.error : "사진 저장 실패");
        return;
      }
      void celebrate();
      await loadWeek();
  ...
  function onCellActivate(dateKey: string) {
    ...
    if (dayIndex === undefined) {
      window.alert(MSG_NOT_CHECK_PERIOD);
      return;
    }
    if (!editable) {
      window.alert("이번 주 기록 마감 시간이 지나 더 이상 수정할 수 없습니다.");
      return;
    }
    if (!me || busy) return;
  ...
  const weekStartLabel = weekStartTz.toLocaleDateString("ko-KR", {
    month: "numeric",
    day: "numeric",
    weekday: "short",
  });
  const weekEndTz = addDays(weekStartTz, 6);
  const weekEndLabel = weekEndTz.toLocaleDateString("ko-KR", {
    month: "numeric",
    day: "numeric",
    weekday: "short",
  });
    ...
    <div className="mx-auto max-w-md px-5 pt-6">
      <header className="mb-6">
        ...
        <p className="mt-2 text-sm leading-relaxed tracking-wide text-stone-500">
          ...
          {!editable ? (
            <span className="ml-1.5 rounded-md bg-stone-100 px-1.5 py-0.5 text-xs font-medium text-stone-500">
              기록 마감
            </span>
          ) : null}
        </p>
        ...
        <p className="mt-2 text-xs tracking-wide text-stone-400">
          이번 주 구간: {weekStartLabel} ~ {weekEndLabel} · 진한 테두리 날만 체크할 수 있어요.
        </p>
      </header>
      ...
      <section className="rounded-2xl border border-stone-200/80 bg-white px-3 py-5 shadow-sm sm:px-4">
        ...
        <div className="mb-3 flex items-center justify-between gap-2">
          <button
            ...
            className="rounded-xl border border-stone-200 bg-white px-3 py-2 text-xs font-medium tracking-wide text-stone-600 shadow-sm transition hover:border-stone-300 hover:bg-stone-50"
          >
            이전 달
          </button>
        ...
        <div className="rounded-xl border border-stone-100 bg-stone-50/40 p-1">
          <div className="grid grid-cols-7 divide-x divide-stone-200/60 pb-0.5">
            {WEEK_HEADERS.map((h) => (
              <div
                key={h}
                className="py-1 text-center text-[10px] font-semibold tracking-wider text-stone-400"
              >
                {h}
              </div>
            ))}
          ...
          {monthCells.map((cell) => {
            ...
            const hit =
              dayIndex !== undefined ? me.days.find((d) => d.dayIndex === dayIndex) : undefined;
            const isToday = todayKey !== "" && cell.dateKey === todayKey;
            const canTapWeek = inActiveWeek && editable && !busy;

            const baseCell =
              "flex aspect-square min-h-[2.35rem] flex-col items-center justify-center rounded-xl text-xs font-medium tabular-nums tracking-tight transition";

            let cellClass = baseCell;
            if (!cell.inDisplayMonth) {
            ...
            if (inActiveWeek) {
              ...
              if (canTapWeek) {
                cellClass += " cursor-pointer active:scale-[0.95]";
              }
            } else {
              cellClass += hit
            ...
            if (isToday && !(inActiveWeek && hit)) {
              cellClass += " ring-1 ring-indigo-400 ring-offset-1";
            }
            ...
            return (
              <div key={cell.dateKey} className="flex flex-col items-center gap-1">
                <button
                  ...
                  aria-label={
                    inActiveWeek
                      ? `${cell.m}월 ${cell.d}일 ${hit ? "운동 취소" : "운동 완료"}`
                      : `${cell.m}월 ${cell.d}일`
                  }
                >
                  {inner}
                ...
                {inActiveWeek && dayIndex !== undefined && canTapWeek && hit ? (
                  <>
                    <input
                      ref={(el) => {
                        fileInputs.current[dayIndex] = el;
                      }}
                      type="file"
                    ...
                    <button
                      type="button"
                      disabled={busy}
                      className="text-[9px] font-medium tracking-wide text-indigo-400 underline decoration-indigo-200 underline-offset-2"
                ...
                {inActiveWeek && dayIndex !== undefined && canTapWeek && !hit && me.completionCount < goal ? (
                  <>
                    <input
                      ref={(el) => {
                        fileInputs.current[100 + dayIndex] = el;
                      }}
                      type="file"
                      accept="image/jpeg,image/png,image/webp,image/gif"
                      className="hidden"
        ...
        {!editable ? (
          ...
          <p className="mt-5 text-center text-xs leading-relaxed tracking-wide text-stone-400">
            이번 주에 해당하는 날(파란 테두리 칸)만 눌러 기록할 수 있습니다.
          </p>
        )}
      </section>
    </div>
